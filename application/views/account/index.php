				...
				<div class="col-md-12">
					...
					<div class="portlet light">
						...
						<div class="portlet-body">
							<table class="table table-bordered table-hover" id="account_detail">
								<thead>
									<tr>
									...
									?>
												<tr>
													...
													<td class="text-center actions">
														...
														<?php echo anchor('account/delete/'.$row['id'].'/index', 'Delete',array('class' => 'btn default btn-xs red','onclick'=>"return confirm('Are you sure to remove this Account?')"));?>
													</td>
												</tr>
									<?php 
											}
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="scroller" style="height:450px" data-always-visible="1" data-rail-visible1="1">
					<div class="col-md-12">
						<div class="row">
							<div class="form-group">
								...
								<div class="col-md-4">
									...
									<select class="form-control input-sm select2me" name="mode" id="mode" required>
										<option value="">- Please Select</option>
						...
						<div class="row">
							<div class="form-group">
								<label class="control-label col-md-3 font-sm">Number</label>
								<div class="col-md-9">
									<input class="form-control form-control-inline input-sm" size="20" maxlength="50" type="text" name="number" id="number" required/>
								</div>
							</div>
						</div>
						<div class="row">
							<div class="form-group">
								<label class="control-label col-md-3 font-sm">Currency</label>
								<div class="col-md-5">
									<select class="form-control select2me input-sm" name="currency_id" id="currency_id">
										<option value="">-- Please Select --</option>
										<?php									 
											if(!empty($currency))
											{
						...
						<div class="row">
							<div class="form-group">
								<label class="control-label col-md-3 font-sm">Profile</label>
								<div class="col-md-9">
									<input class="form-control form-control-inline input-sm" size="20" maxlength="50" type="text" name="profile" id="profile"/>
								</div>
							</div>
						</div>
						<div class="row">
							<div class="form-group">
								<label class="control-label col-md-3 font-sm">Security</label>
								<div class="col-md-9">
									<textarea name="security" id="security" class="col-md-12 form-control form-control-inline input-sm"></textarea>
								</div>
							</div>
						</div>
						<div class="row">
							<div class="form-group">
								<label class="control-label col-md-3 font-sm">Contact</label>
								<div class="col-md-9">
									<textarea name="contact" id="contact" class="col-md-12 form-control form-control-inline input-sm"></textarea>
								</div>
							</div>
						</div>
						<div class="row">
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn red" onclick="save_account();">Save</button>
			</div>
		</div>
	</div>
</div>
...
<script>
	...
	function save_account()
	{
		...
		if(mode!="" && currency_id!= "" && number!="")
		{
			$.ajax({
		        type:"POST",
		        url: "<?php echo site_url('account/save_account'); ?>",
		        data:{"id":id,"currency_id":currency_id,"mode":mode,"profile":profile,"security":security,"contact":contact,"email":email,"remark":remark,"number":number,"status":status},
		        success: function(response){
		        	console.log(response);
		        	$("#account_detail").load(location.href + " #account_detail");
		        	$('#add_item').modal('hide');
		        }
		    });
		}
	...
	$(document).on("click", ".add_item_modal", function () {
		...
		$.ajax({
	        type:"POST",
	        url: "<?php echo site_url('account/get_account_detail'); ?>",
	        data:{"id":id},
	        success: function(response){
	        	response = JSON.parse(response);
	        	console.log(id);
	        	set_select2me_text('#mode',response.mode,response.mode);
	        	set_select2me_text('#currency_id',response.currency_id,response.code);
