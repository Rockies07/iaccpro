...
class utility extends User_Access_Controller 
{
	...
	function update_calendar_drag($string_data)
	{
		...
		foreach ($get_schedule_detail as $schedule_item) 
		{
			...
			$current_start=date('Y-m-d H:i:s',strtotime("+".$sec." second", strtotime($get_start)));
			
			if($get_end!="0000-00-00 00:00:00")
			{
				$current_end=date('Y-m-d H:i:s',strtotime("+".$sec." second", strtotime($get_end)));
			}
			else
			{
	...
	function update_session($session, $value)
	{
	...
	function update_session_menu($session, $value)
	{	
		if($this->session->userdata('sys_menu')!='')
		{
